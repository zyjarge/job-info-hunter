...
class BossLogin:
    ...
    def save_cookies(self, cookies):
        ...
        cookie_data = {"cookies": cookies, "expires": expires}
    ...
    async def login_by_qrcode(self) -> bool:
        ...
        try:
            ...
            # 切换到扫码登录
            await self._switch_to_qrcode_login()

            # 等待并获取二维码
            qr_code_url = await self._get_qrcode()
            if qr_code_url:
                logger.info(f"微信小程序二维码链接: {qr_code_url}")
    ...
    async def _switch_to_qrcode_login(self):
        """切换到扫码登录"""
    ...
    async def _get_qrcode(self) -> str:
        """获取二维码图片链接"""
        try:
            # 等待小程序二维码出现
            ...
            # 获取二维码图片链接
            qrcode_element = await self.page.query_selector(
                BossSelectors.MINI_APP_QRCODE
            )
            if qrcode_element:
                return await qrcode_element.get_attribute("src")
    ...
    async def _wait_for_login_success(self, timeout: int = 300000) -> bool:
        """等待用户扫码登录成功
        ...
        try:
            ...
            await self.page.wait_for_selector(BossSelectors.USER_MENU, timeout=timeout)
            await self.page.wait_for_selector(
                f"{BossSelectors.USER_MENU} .label-text", timeout=5000
            )
            await self.page.wait_for_selector(
                f"{BossSelectors.USER_MENU} img", timeout=5000
            )
