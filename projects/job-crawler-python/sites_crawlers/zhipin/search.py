...
import random
from sites_crawlers.zhipin.login import BossLogin
from utils.logger import setup_logger
from asyncio import Semaphore
import argparse  # 添加参数解析支持
import sys
from pathlib import Path

sys.path.append(str(Path(__file__).parent))
# 获取logger实例
logger = setup_logger("zhipin_search")
...
class BossSearchSelectors:
    ...
    DETAIL_COMPANY_DESC = (
        ".detail-section-item.company-info-box .job-sec-text"  # 公司介绍
    )
...
class BossSearcher:
    ...
    async def search(
        self, keyword: str, location: str = None, limit: int = 100
    ) -> List[Dict]:
        """执行搜索并返回结果"""
        try:
            logger.info(
                f"开始搜索任务 - 关键词: {keyword}, 地区: {location}, 数据上限: {limit}"
            )
    ...
    async def _get_search_results_by_limit(self, limit: int) -> List[Dict]:
        ...
        logger.info(f"开始获取搜索结果，计划抓�� {limit} 条数据")

        while len(results) < limit:
            logger.info(
                f"正在抓取第 {current_page} 页 (当前已获取 {len(results)}/{limit} 条)"
            )
            ...
            logger.debug(
                f"第 {current_page} 页抓取完成，当前共有 {len(results)}/{limit} 条数据"
            )
    ...
    async def _extract_page_jobs(self, remaining_limit: int = None) -> List[Dict]:
            ...
            async with self.semaphore:
                ...
                if result:
                    ...
                    if company_info and company_info.get("company_name"):
                        logger.debug(
                            f"成功获取职位信息: {result.get('job_info', {}).get('title')}"
                        )
                        return result
                    else:
        ...
        while processed_count < len(job_urls) and (
            remaining_limit is None or len(valid_jobs) < remaining_limit
        ):
            # 计算本批次需要处理的数量
            remaining_needed = (
                remaining_limit - len(valid_jobs) if remaining_limit else float("inf")
            )
            current_batch_size = min(
                batch_size, len(job_urls) - processed_count, remaining_needed
            )
            ...
            # 获取当前批次的URL
            current_batch = job_urls[
                processed_count : processed_count + current_batch_size
            ]
            logger.debug(
                f"处理第 {processed_count//batch_size + 1} 批，"
    ...
    async def _get_job_url(self, job_card) -> tuple[str, str]:
        ...
        try:
            ...
            if not link_element:
                logger.warning(
                    f"未找到职位链接元素 (选择器: {BossSearchSelectors.JOB_LINK})"
                )
                return "", ""
            ...
            try:
                ...
                job_info = {
                    "title": await self._get_text(
                        page, BossSearchSelectors.DETAIL_JOB_TITLE
                    ),
                    "salary": await self._get_text(
                        page, BossSearchSelectors.DETAIL_SALARY
                    ),
                    "description": await self._get_text(
                        page, BossSearchSelectors.DETAIL_JOB_DESC
                    ),
                    "requirements": await self._get_text(
                        page, BossSearchSelectors.DETAIL_REQUIREMENTS
                    ),
                    "location": await self._get_text(
                        page, BossSearchSelectors.DETAIL_LOCATION
                    ),
                    "experience": await self._get_text(
                        page, BossSearchSelectors.DETAIL_EXPERIENCE
                    ),
                    "education": await self._get_text(
                        page, BossSearchSelectors.DETAIL_EDUCATION
                    ),
                    "benefits": await self._get_tags(
                        page, BossSearchSelectors.DETAIL_BENEFITS
                    ),
                ...
                hr_info = {
                    "name": (
                        await self._get_text(page, BossSearchSelectors.HR_NAME)
                    ).split("\n")[0],
                    "title": (
                        await self._get_text(page, BossSearchSelectors.HR_TITLE)
                    ).split("\n·\n")[-1],
                    "active": await self._get_text(page, BossSearchSelectors.HR_ACTIVE),
                    "avatar": await self._get_image_url(
                        page, BossSearchSelectors.HR_AVATAR
                    ),
                }
                ...
                detail_info = {
                    ...
                    "update_time": (
                        await self._get_text(
                            page, BossSearchSelectors.DETAIL_UPDATE_TIME
                        )
                    )
                    .replace("页面更新时间：", "")
                    .strip(),
    ...
    def save_results(
        ...
        if not filename:
            ...
            location = (
                location.replace("/", "_").replace("\\", "_").strip()
                if location
                else "全国"
            )
            # 确保data目录存在
            ...
            filename = os.path.join(
                data_dir, f"zhipin_{keyword}_{location}_{current_time}.json"
            )
    ...
    async def _fetch_job_detail_with_retry(
        self, url: str, job_id: str, max_retries: int = 3
    ) -> Optional[Dict]:
        """带重试机制的详情页获取"""
    ...
    async def _check_is_headhunter(self, page) -> bool:
        ...
        try:
            # 检查是否存在猎头图标
            headhunter_icon = await page.query_selector(
                BossSearchSelectors.DETAIL_HEADHUNTER_ICON
            )
        ...
        try:
            ...
            try:
                await page.wait_for_selector(
                    BossSearchSelectors.DETAIL_COMPANY_NAME, timeout=3000
                )
            except Exception:
            ...
            company_name = await self._get_text(
                page, BossSearchSelectors.DETAIL_COMPANY_NAME
            )
            ...
            legal_representative = await self._get_text(
                page, BossSearchSelectors.DETAIL_LEGAL_REPRESENTATIVE
            )
            legal_representative = legal_representative.replace(
                "法定代表人", ""
            ).strip()

            establish_date = await self._get_text(
                page, BossSearchSelectors.DETAIL_ESTABLISH_DATE
            )
            establish_date = establish_date.replace("成立日期", "").strip()

            company_type = await self._get_text(
                page, BossSearchSelectors.DETAIL_COMPANY_TYPE
            )
            company_type = company_type.replace("企业类型", "").strip()

            business_status = await self._get_text(
                page, BossSearchSelectors.DETAIL_BUSINESS_STATUS
            )
            business_status = business_status.replace("经营状态", "").strip()

            registered_capital = await self._get_text(
                page, BossSearchSelectors.DETAIL_REGISTERED_CAPITAL
            )
            registered_capital = registered_capital.replace("注册资金", "").strip()

            company_desc = await self._get_text(
                page, BossSearchSelectors.DETAIL_COMPANY_DESC
            )
            company_address = await self._get_text(
                page, BossSearchSelectors.DETAIL_COMPANY_ADDRESS
            )
...
async def main(keyword: str = "数据工程师", location: str = "北京", limit: int = 10):
    """主函数，支持参数传入"""
    try:
        # 初始化登录实例
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="BOSS直聘职位搜索工具")
    parser.add_argument(
        "--keyword",
        "-k",
        type=str,
        default="数据工程师",
        help='搜索关键词，默认为"数据工程师"',
    )
    parser.add_argument(
        "--location", "-l", type=str, default="北京", help='地区，默认为"北京"'
    )
    parser.add_argument(
        "--limit", "-n", type=int, default=10, help="获取数量上限，默认为10"
    )
...
if __name__ == "__main__":
    ...
    # 运行主函数
    asyncio.run(main(keyword=args.keyword, location=args.location, limit=args.limit))
