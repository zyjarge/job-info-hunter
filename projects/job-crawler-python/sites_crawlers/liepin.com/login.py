...
class LiepinLogin:
    ...
    async def init_browser(self):
        ...
        self.browser = await self.playwright.chromium.launch(headless=config['headless'], args=browser_args)
    ...
    def _is_cookies_expired(self, cookies: list) -> bool:
        ...
        try:
            ...
            for cookie in cookies:
                # 只检查关键cookie的过期时间
                if cookie.get('name') in ['__login_token', 'access_token', 'user_token']:
    ...
    async def check_login_status(self) -> bool:
        ...
        logger.debug("检查登录状态")
        try:
            await self.page.wait_for_selector('#header-quick-menu-user-info', timeout=5000)
            await self.page.wait_for_selector('.header-quick-menu-username', timeout=5000)
            logger.debug("用户已登录")
            return True
        except Exception as e:
