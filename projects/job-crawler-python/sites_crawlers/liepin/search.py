...
import random
from sites_crawlers.liepin.login import LiepinLogin, logger
...
class LiepinSearchSelectors:
    ...
    DETAIL_REGISTERED_CAPITAL = (
        ".register-info .label-box:nth-child(2) .text"  # 注册资本
    )
...
class LiepinSearcher:
    ...
    async def search(
        self, keyword: str, location: str = None, limit: int = 10
    ) -> List[Dict]:
        """执行搜索并返回结果"""
        try:
            logger.info(
    ...
    async def _perform_search(self):
        ...
        try:
            ...
            error_delay = random.uniform(
                self.config.DELAY_AFTER_ERROR_MIN, self.config.DELAY_AFTER_ERROR_MAX
            )
        ...
        try:
            while len(results) < limit:
                ...
                for job in page_jobs:
                    if "url" in job:
                        logger.debug(
                            f"创建获取详情任务: {job.get('title', 'Unknown')} - {job['url']}"
                        )
                        task = asyncio.create_task(
                            self._fetch_job_detail_with_retry(job["url"])
                        )
                        tasks.append(task)
                ...
                if tasks:
                    ...
                    for result in batch_results:
                        ...
                        if result:
                            logger.debug(
                                f"成功获取职位详情: {result.get('job_info', {}).get('title', 'Unknown')}"
                            )
            ...
            try:
                ...
                job_info = {
                    "title": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_JOB_TITLE
                    ),
                    "salary": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_SALARY
                    ),
                    "company": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_COMPANY_NAME
                    ),
                    "description": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_JOB_DESC
                    ),
                    "location": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_LOCATION
                    ),
                    "experience": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_EXPERIENCE
                    ),
                    "education": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_EDUCATION
                    ),
                    "benefits": await self._get_tags(
                        page, LiepinSearchSelectors.DETAIL_BENEFITS
                    ),
                    "detail_url": url,
                }
                ...
                company_info = {
                    ...
                    "establish_date": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_ESTABLISH_DATE
                    ),
                    "company_type": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_COMPANY_TYPE
                    ),
                    "business_status": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_BUSINESS_STATUS
                    ),
                    "registered_capital": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_REGISTERED_CAPITAL
                    ),
                    "company_desc": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_COMPANY_DESC
                    ),
                    "company_address": await self._get_text(
                        page, LiepinSearchSelectors.DETAIL_COMPANY_ADDRESS
                    ),
                }
                ...
                hr_info = {
                    ...
                    "company": await self._get_text(
                        page, LiepinSearchSelectors.HR_COMPANY
                    ),
                    "avatar": await self._get_image_url(
                        page, LiepinSearchSelectors.HR_AVATAR
                    ),
                }
    ...
    async def _extract_page_jobs(self, remaining_limit: int = None) -> List[Dict]:
        ...
        try:
            ...
            job_cards = await self.page.query_selector_all(
                LiepinSearchSelectors.JOB_ITEMS
            )
    ...
    def save_results(
        ...
        if not filename:
            ...
            filename = os.path.join(
                data_dir, f"liepin_{keyword}_{location}_{current_time}.json"
            )
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="猎聘网职位搜索工具")
    parser.add_argument(
        "--keyword",
        "-k",
        type=str,
